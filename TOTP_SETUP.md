...
从 `2.0.2` 开始，权限放行、重试本轮等应用内操作不再额外使用严格 same-origin 判断拦截。公网安全边界统一交给访问码、Authenticator、插件 Token 和反向代理 HTTPS / 限速；这样可以避免 NAS、反向代理、移动浏览器壳因为 Origin / Host 不一致导致授权按钮误报 `same-origin request required`。
...
Claude Code Web 只在直连节点来自本机回环地址时信任转发头。局域网、私网代理和公网客户端都必须完成设备认证，无法靠伪造 `X-Forwarded-For` 把自己标记成本机来源。
...
这是预期行为。只有 `127.0.0.1` 和 `::1` 不要求验证码。
...
- Authenticator 只能从电脑本机的回环地址管理页面配置。
- 先开启「手机访问」，再点击「开始设置」。
- 刷新页面后确认后端版本为 `2.0.2`。
